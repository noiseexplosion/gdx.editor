...
public class HeightField implements Disposable {
    ...
    public HeightField(boolean isStatic, int width, int height, boolean smooth, VertexAttributes attributes) {
        ...
        mesh = new Mesh(isStatic, numVertices, numIndices, attributes);
    ...
    private void updateSmooth () {
        for (int x = 0; x < width; ++x) {
            for (int y = 0; y < height; ++y) {
                MeshPartBuilder.VertexInfo v = this.getVertexAt(vertex00, x, y);
                this.getWeightedNormalAt(v.normal, x, y);
                this.setVertex(y * width + x, v);
            }
        }
    ...
    private void updateSimple () {
        for (int x = 0; x < width; ++x) {
            for (int y = 0; y < height; ++y) {
                this.setVertex(y * width + x, this.getVertexAt(vertex00, x, y));
            }
        }
        mesh.setVertices(vertices);
    }
    ...
    private void updateSharp () {
        ...
        for (int y = 0; y < h; ++y) {
            for (int x = 0; x < w; ++x) {
                ...
                MeshPartBuilder.VertexInfo v00 = this.getVertexAt(vertex00, x, y);
                MeshPartBuilder.VertexInfo v10 = this.getVertexAt(vertex10, x + 1, y);
                MeshPartBuilder.VertexInfo v01 = this.getVertexAt(vertex01, x, y + 1);
                MeshPartBuilder.VertexInfo v11 = this.getVertexAt(vertex11, x + 1, y + 1);
                ...
                this.setVertex(c00, v00);
                this.setVertex(c10, v10);
                this.setVertex(c01, v01);
                this.setVertex(c11, v11);
            }
        }
        mesh.setVertices(vertices);
    }
    ...
    protected MeshPartBuilder.VertexInfo getVertexAt (final MeshPartBuilder.VertexInfo out, int x, int y) {
...
// A more accurate approach would be to weight the normal of the actual triangles.
        ...
        Vector3 center = this.getPositionAt(tmpV2, x, y);
        Vector3 left = x > 0 ? this.getPositionAt(tmpV3, x - 1, y) : null;
        Vector3 right = x < (width - 1) ? this.getPositionAt(tmpV4, x + 1, y) : null;
        Vector3 bottom = y > 0 ? this.getPositionAt(tmpV5, x, y - 1) : null;
        Vector3 top = y < (height - 1) ? this.getPositionAt(tmpV6, x, y + 1) : null;
    ...
    public void set (final Pixmap map) {
        ...
        this.set(map.getPixels(), map.getFormat());
    }

    public void set (final ByteBuffer colorData, final Pixmap.Format format) {
        this.set(HeightField.heightColorsToMap(colorData, format, width, height));
    }
